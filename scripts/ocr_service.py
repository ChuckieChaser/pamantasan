...
def create_searchable_pdf(ocr_result: dict, title: str = "Scanned Document") -> bytes:
    ...
    # PAGE 2: Institutional OCR Transcript Sheet (Clean, Searchable & Auditable)
    # A4 dimensions: 595 x 842 pt
    page2 = doc.new_page(width=595, height=842)
    margin = 40
    
    # Header styling
    header_rect = fitz.Rect(margin, 35, 595 - margin, 95)
    page2.draw_rect(header_rect, color=(0.12, 0.35, 0.22), fill=(0.94, 0.97, 0.94))
    
    page2.insert_text(
        fitz.Point(margin + 15, 60),
        "PAMANTASAN RECORDS MANAGEMENT SYSTEM",
        fontsize=13,
        fontname="helv",
        color=(0.12, 0.35, 0.22)
    )
    
    subtitle = f"Automated PaddleOCR Text Layer & Document Indexing - {title}"
    page2.insert_text(
        fitz.Point(margin + 15, 78),
        subtitle[:75],
        fontsize=8.5,
        fontname="helv",
        color=(0.3, 0.3, 0.3)
    ...
    conf_pct = int(ocr_result["average_confidence"] * 100)
    meta_text = (
        f"Confidence Score: {conf_pct}%   |   "
        f"Detected Lines: {ocr_result['lines_count']}   |   "
        f"Engine: PaddleOCR v3.7"
    )
    page2.insert_text(
        fitz.Point(margin, 115),
        meta_text,
        fontsize=9,
        fontname="helv",
        color=(0.2, 0.2, 0.2)
    ...
    # Divider rule
    page2.draw_line(fitz.Point(margin, 125), fitz.Point(595 - margin, 125), color=(0.8, 0.8, 0.8))
    
    # Body: Structured extracted text
    body_rect = fitz.Rect(margin, 140, 595 - margin, 780)
    full_text = ocr_result["full_text"] if ocr_result["full_text"] else "[No readable text detected in source image]"
    ...
    # Footer
    page2.insert_text(
        fitz.Point(margin, 815),
        "Certified Document Transcription - Pamantasan Institutional Repository",
        fontsize=7.5,
        fontname="helv",
        color=(0.5, 0.5, 0.5)
    )
